...
const handleRegister = async () => {
  ...
  try {
    ...
    if (Array.isArray(resMessage)) {
      error.value = resMessage.join(', ');
    } else {
      error.value = resMessage || 'Error al registrarse';
    }
  } finally {
    loading.value = false;
  }
  ...
  <div class="min-h-screen flex items-center justify-center bg-[#121212] px-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-[400px] bg-[#1a1a1a] rounded-2xl p-8 border border-[#2a2a2a] shadow-2xl relative z-10">
      <div class="flex flex-col items-center mb-8">
        ...
        <p class="text-gray-400 mt-2 text-sm text-center">Join the community of movie lovers.</p>
      </div>
      ...
      <form @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div>
          ...
          <input 
            ...
            type="text" 
            required 
            class="w-full bg-[#121212] border border-[#3a3a3a] rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#a855f7] transition-colors text-sm"
      ...
      <p class="text-center text-gray-400 text-sm mt-8">
        ...
        <router-link to="/login" class="text-[#d4d444] hover:underline font-medium">Sign in</router-link>
      </p>
    </div>
  </div>
</template>
